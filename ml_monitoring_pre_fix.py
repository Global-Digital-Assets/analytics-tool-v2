...
class MLMonitoringSystem:
    ...
    def check_retrain_conditions(self, evaluation_results: Dict) -> Dict:
        ...
        baseline_auc = evaluation_results.get('baseline_auc', 0.821)  # From training metadata
        
        # 🎯 INSTITUTIONAL DRIFT DETECTION
        # Trigger: 7-day rolling AUC < (CV AUC - 3pp)
        auc_degradation = baseline_auc - current_perf['auc']
        drift_threshold_institutional = 0.03  # 3 percentage points
        ...
        if retrain_decision["should_retrain"]:
            ...
            logger.info(f"✅ Model performance stable - AUC: {current_perf['auc']:.3f} (degradation: {auc_degradation:.3f})")
