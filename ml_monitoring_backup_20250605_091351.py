...
class MLMonitoringSystem:
    ...
    def evaluate_current_model(self, days_back: int = 7) -> Dict:
        ...
        try:
            ...
            training_accuracy = metadata['performance']['accuracy']
            training_auc = metadata['performance']['auc']
            
            accuracy_drift = training_accuracy - current_accuracy
            auc_drift = training_auc - current_auc
            ...
            evaluation_results = {
                ...
                "alerts": self._generate_alerts(current_accuracy, current_auc, accuracy_drift, auc_drift)
            }
            
            # Log to monitoring database
            self._log_performance(evaluation_results)
            
            logger.info(f"📊 Model Evaluation: Acc={current_accuracy:.3f} (drift: {accuracy_drift:+.3f}), AUC={current_auc:.3f} (drift: {auc_drift:+.3f})")
    ...
    def check_retrain_conditions(self, evaluation_results: Dict) -> bool:
        ...
        accuracy_below_threshold = current_perf['accuracy'] < self.accuracy_threshold
        auc_below_threshold = current_perf['auc'] < self.auc_threshold
        
        # Check drift thresholds
        significant_accuracy_drift = drift['accuracy_drift'] > self.drift_threshold
        significant_auc_drift = drift['auc_drift'] > self.drift_threshold
        
        should_retrain = (
            accuracy_below_threshold or 
            auc_below_threshold or 
            significant_accuracy_drift or 
            significant_auc_drift
        )
        
        if should_retrain:
            reason = []
            if accuracy_below_threshold:
                reason.append(f"accuracy below {self.accuracy_threshold}")
            if auc_below_threshold:
                reason.append(f"AUC below {self.auc_threshold}")
            if significant_accuracy_drift:
                reason.append(f"accuracy drift {drift['accuracy_drift']:.3f}")
            if significant_auc_drift:
                reason.append(f"AUC drift {drift['auc_drift']:.3f}")
            
            logger.warning(f"🚨 RETRAIN TRIGGERED: {', '.join(reason)}")
    ...
    def auto_retrain_pipeline(self) -> bool:
        ...
        try:
            ...
            if not self.check_retrain_conditions(evaluation_results):
                logger.info("✅ Model performance acceptable, no retrain needed")
                return True
            
            # Trigger retrain
            logger.info("🔄 Starting automated retrain...")
            ...
            if retrain_results:
                logger.info("✅ Automated retrain completed successfully")
                ...
                logger.error("❌ Automated retrain failed")
                return False
