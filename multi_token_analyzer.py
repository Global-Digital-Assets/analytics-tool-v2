...
# Utility to parse boolean env vars
...
ENABLE_ADAPTIVE_TIERS = _env_flag("ENABLE_ADAPTIVE_TIERS", "False")
ENABLE_MICROSTRUCTURE_STATS = _env_flag("ENABLE_MICROSTRUCTURE_STATS", "False")
ENABLE_REGIME_THRESHOLDS = _env_flag("ENABLE_REGIME_THRESHOLDS", "False")

# Threshold matrix for regime-based thresholds
THRESHOLD_MATRIX = {
    'trending_volatile': {'entry': 1.2, 'noise': 0.8},
    'trending_calm': {'entry': 1.5, 'noise': 0.5},
    'choppy': {'entry': 2.0, 'noise': 1.0},
    'squeeze': {'entry': 1.0, 'noise': 0.3},
}
...
try:
    from config import (
        ...
        MEME_TOKENS as CFG_MEME_TOKENS,
    ...
    # Legacy 5-token fallback removed – config import should always succeed
    DEFAULT_TOKENS = []
    MEME_TOKENS = []
...
THRESHOLD_CONFIG_PATH = os.path.join(SCRIPT_DIR, 'thresholds.yml')
if os.path.exists(THRESHOLD_CONFIG_PATH):
    with open(THRESHOLD_CONFIG_PATH, 'r') as f:
        _threshold_cfg = yaml.safe_load(f) or {}
else:
    _threshold_cfg = {}
...
TIERED_THRESHOLDS = {
    cat: _threshold_cfg.get(cat, DEFAULT_THRESHOLDS[cat])
    for cat in DEFAULT_THRESHOLDS
}
...
class RegimeDetector:
    ...
    def _get_db_connection(self):
        return sqlite3.connect(self.db_path, timeout=60, check_same_thread=False)
    ...
    # --- New advanced regime classification ---
    def classify_regime_adv(self, symbol="BTCUSDT") -> str:
        """Return one of trending_volatile, trending_calm, choppy, squeeze."""
        try:
            conn = self._get_db_connection()
            query = """SELECT close FROM candles_900s WHERE symbol = ? ORDER BY timestamp DESC LIMIT 120"""
            ...
            vol = returns.std()*100
            vol_med = returns.rolling(60).std().median()*100 if len(returns)>=60 else vol
            vol_percentile = 1.0 if vol_med == 0 else min(vol/vol_med, 2.0)
            sma = df['close'].rolling(window=60).mean().iloc[-1]
            trend_strength = abs(df['close'].iloc[-1]-sma)/sma if sma>0 else 0
            volatile = vol > 1.5
            trending = trend_strength > 0.01
...
class EnhancedTokenAnalyzer:
    ...
    async def _fetch_historical_data_from_db(self, symbol: str) -> pd.DataFrame:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path, timeout=60, check_same_thread=False) as conn:
    ...
    async def analyze_token_async(self, symbol: str, session: aiohttp.ClientSession) -> Optional[TokenOpportunity]:
        ...
        try:
            ...
            # New: 5-min price change & volatility-relative tier
            price_change_5m = ((current_price - df["close"].iloc[-2]) / df["close"].iloc[-2]) if len(df) >= 2 else 0.0
            rolling_std_20 = df["close"].pct_change().tail(21).std()  # 20 recent 5-m returns
            signal_tier = self._get_signal_tier(price_change_5m, rolling_std_20) if ENABLE_ADAPTIVE_TIERS else "LEGACY"
            if ENABLE_ADAPTIVE_TIERS:
                logger.debug(f"{symbol} move_sigma tier {signal_tier}")

            # Enhanced feature extraction
            features = self.scoring_engine.extract_features(df, current_price)

            # --- Microstructure stats (logging only) ---
            if ENABLE_MICROSTRUCTURE_STATS:
                spread, imb, max_imb = self._get_micro_stats(symbol)
                logger.debug(f"{symbol} microstructure spread {spread:.6f} avgImb {imb:.4f}")

            # Guard: if ALL engineered technical features are NaN or 0, skip token to avoid polluting score distribution
            if all((val is None or (isinstance(val, (int, float)) and (np.isnan(val) or val == 0))) for val in features):
                logger.warning(f"⚠️ Skipping {symbol}: all technical features are NaN/zero (stale or missing data)")
                return None
            ...
            # Determine thresholds per category
            category = 'memes' if symbol in MEME_TOKENS else 'majors'
            long_thresh = TIERED_THRESHOLDS[category]['long']
            short_thresh = TIERED_THRESHOLDS[category]['short']
            if ENABLE_REGIME_THRESHOLDS and regime_info['regime'] in THRESHOLD_MATRIX:
                long_thresh *= THRESHOLD_MATRIX[regime_info['regime']]['entry']
                short_thresh *= THRESHOLD_MATRIX[regime_info['regime']]['entry']
            
            # Binary decisions using tiered thresholds
            simple_long_action_val = "BUY_LONG" if final_long_score >= long_thresh else "DON'T_BUY_LONG"
            simple_short_action_val = "SHORT" if final_short_score >= short_thresh else "DON'T_SHORT"

            # Noise filter using sigma tiers
            if ENABLE_ADAPTIVE_TIERS and ENABLE_REGIME_THRESHOLDS and regime_info['regime'] in THRESHOLD_MATRIX:
                if abs(price_change_5m) < THRESHOLD_MATRIX[regime_info['regime']]['noise']:
                    direction = "NEUTRAL"
                    entry_recommendation = "WAIT"
                else:
                    # Determine primary direction using tiered thresholds (below)
                    pass
            else:
                # Determine primary direction using tiered thresholds
                if final_long_score > final_short_score and final_long_score >= long_thresh:
                    direction = "LONG"
                    entry_recommendation = "BUY_LONG"
                elif final_short_score > final_long_score and final_short_score >= short_thresh:
                    direction = "SHORT"
            ...
            # Volatility-scaled stop-loss (applied to all tokens)
            base_sl_pct = 8.0 if category == 'memes' else 12.0  # tighter for high-beta memes
            stop_loss_pct = max(volatility * 1.2, base_sl_pct) if volatility > 0 else base_sl_pct
            ...
            logger.info(f"📊 {symbol}: {scoring_method.upper()} | Raw: L={raw_long_score:.1f}/S={raw_short_score:.1f} | "
                       ...
                       f"Action: {simple_long_action_val}/{simple_short_action_val} | SL={stop_loss_pct:.1f}%")
    ...
    def _get_signal_tier(self, price_change: float, rolling_std: float) -> str:
        """Classify move into tier A/B/C/NONE based on sigma move."""
        if rolling_std == 0:
            return "NONE"
        move_sigma = abs(price_change) / rolling_std
        if move_sigma > 3.0:
            return "A"
        elif move_sigma > 2.0:
            return "B"
    ...
    def _get_micro_stats(self, symbol: str):
        """Fetch latest microstructure stats for symbol (spread, imbalance, max_abs)."""
        if not ENABLE_MICROSTRUCTURE_STATS:
            return 0.0, 0.0, 0.0
        try:
            if self.MICRO_DB_CONN is None:
                self.MICRO_DB_CONN = sqlite3.connect(DB_PATH, timeout=60, check_same_thread=False)
            cur = self.MICRO_DB_CONN.cursor()
            cur.execute(
                """
                SELECT avg_spread, avg_imbalance, max_abs_imbalance
                FROM microstructure_stats WHERE symbol = ?
            ...
            logger.debug(f"microstructure stats fetch error for {symbol}: {exc}")
        return 0.0, 0.0, 0.0
...
async def main():
    ...
    regime_class = analyzer.regime_detector.classify_regime_adv()
    logger.info(f"Market Regime Detected: {market_regime_info}")
    logger.info(f"Regime Class: {regime_class}")
